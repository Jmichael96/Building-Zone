...
$(document).on("click", "#select", function () {
  ...
  var $results = $("#results"), str = '<form data-id=' + data[i]._id + 'method="POST">' +
    '<div class="row">' +
    '<div id="datepicker" class="col s4">' +
    '<input id="invoice-date" placeholder="Date" name="date" type="text" class="datepicker black-text">' +
    '</div>' +
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $("#action-btn").html("<button id='submit-update' data-id='" + data._id + "'class='btn submit pulse'>update</button>");
